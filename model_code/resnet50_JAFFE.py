...
# ======= 模型建立 =======
model = resnet50(weights=None).to(device)  # 不要再載入 ImageNet 預訓練
in_features = model.fc.in_features
model.fc = nn.Linear(in_features, num_classes).to(device)

# 載入已訓練好的模型權重（確認路徑正確）
model.load_state_dict(torch.load("resnet50_emotion_RAF-DB.pth", map_location=device))
...
unfreeze_rate = 2
resnet_layers = [model.layer4, model.layer3, model.layer2, model.layer1]  # 解凍順序

def get_layerwise_lr_params(model, base_lr, decay_factor=0.8):
    params = []
    decay = 1.0
    for layer in reversed(resnet_layers):  # 越深層的層學習率越高
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_val_acc:
        ...
        counter = 0
        torch.save(model.state_dict(), 'resnet50_emotion_JAFFE.pth')
        print("✅ 驗證準確率提升，已儲存模型")
    else:
        counter += 1
